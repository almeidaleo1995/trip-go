...
| **(achado em T19, fora do escopo desta feature)** `getSnapshot`'s roteiro query não usa `to_char` para `itinerary_events.ocorre_em` como já faz para `itinerary_days.dia` — o valor chega no `/api/snapshot` com sufixo `Z` e milissegundos (`"2026-12-30T09:00:00.000Z"`), formato que `parseData` (lib/derive.ts) não reconhece (regex exige `YYYY-MM-DDTHH:MM[:SS]` sem `Z`) | `lib/db.ts` (query do roteiro em `getSnapshot`), `lib/derive.ts:41-58` (`parseData`) | Toda chamada de `parseData(evento.ocorre_em)` no app (`proximoCompromisso`, `ordenarEventos`, `resumoDoDia`, e a nova `Dicas`) recebe `null` e degrada em silêncio — a ordenação por data pode estar mascarada pelo `ORDER BY` do SQL, não pelo código JS | `Dicas` (T19) contorna localmente com `new Date(ocorre_em)` em vez de `parseData`, seguro porque a string já vem com `Z` (sem a ambiguidade que `parseData` existe para evitar). Correção na raiz (aplicar `to_char` na query, igual já feito para `dia`) fica fora desta feature — reportado ao usuário, não decidido em silêncio. |
