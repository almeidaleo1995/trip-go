...
### AD-011 - Remover o assistente de IA; a skill passa a escrever no app
**Data:** 2026-09-02
**Contexto:** A AD-010 apostou numa dependencia que quem opera o app precisa manter: uma chave da Anthropic ativa, com fatura, servindo cinco pessoas que nao veem a conta. O usuario decidiu que nao vai manter isso ("remova tudo isso que eu nao vou conseguir fazer") e pediu que o que o assistente fazia de util -- montar a viagem inteira, o roteiro, o guia -- acontecesse pela SKILL, no Claude Code: ele pede ali, a skill monta, e o dado ja sobe no app para abrir na hora. Com duas condicoes: ela nunca altera codigo, e tem que se manter atualizada em relacao ao sistema.
**Decisao:** Sai o modulo inteiro: as 4 rotas `/api/assistente`, `lib/assistente.ts`, `lib/consumo.ts`, `lib/voz.ts`, os 5 componentes, `config/precos.ts`, a tabela `ai_usage`, o balde de rate limit proprio, `ANTHROPIC_API_KEY`/`ANTHROPIC_ADMIN_KEY`/`OPERADOR_EMAILS` e a dependencia `@anthropic-ai/sdk`. A skill `roteiro-trip-go` (1.4.0) ganha tres scripts: `campos.mjs` (a lista de campos viva, lida de `SECOES_ARQUIVO`), `subir.mjs` (grava no app: `--nova` pelo `importarViagem` de `/api/import`, `--viagem <id>` somando numa existente) e `desfazer.mjs` (reverte a carga pelo `lote`).
**Razao:** O que a AD-010 comprou de verdade nao foi o modelo, foi o CAMINHO -- descrever a viagem inteira de uma vez em vez de item por item na tela. Esse caminho ja existia fora do app, na skill, e faltava so a ultima perna: em vez de instruir "escreva um script descartavel e apague depois", a skill agora tem uma gravadora de verdade. Nao ha chave, nao ha fatura, nao ha segredo novo, e o julgamento continua acontecendo onde ja acontecia.
**Consequencia:** Quatro, todas deliberadas. (1) A skill NAO TEM PODER PROPRIO: toda linha passa por `exigirViagem` + `autorizar` + `aplicar`, com o `Acesso` da conta de quem pediu -- visualizador leva 403, quem nao participa leva 404, e ela nao consegue fazer nada que a pessoa ja nao pudesse fazer pela tela. E a mesma propriedade que a AD-010 tinha, sem o modelo. (2) SO DADO, NUNCA CODIGO: `lib/skill.test.ts` falha se os scripts de banco ganharem escrita de arquivo ou `exec`/`spawn`. A regra e verificavel, nao uma promessa -- e essa era a condicao do usuario. (3) Toda carga e marcada (`origem = 'skill'`) e agrupada num `lote` desfazivel; escrever numa viagem em uso sem caminho de volta nao seria aceitavel. (4) A skill se atualiza perguntando ao schema (`campos.mjs`) em vez de reler a propria documentacao -- o zod descarta chave desconhecida em silencio, entao uma secao renomeada importaria vazia sem erro nenhum.
**O que se perdeu, honestamente:** conversar sobre a viagem em linguagem natural DENTRO do app, no celular, offline do desktop. O julgamento agora mora no Claude Code, na maquina de quem organiza. Para quem esta viajando, o app continua sendo edicao pela tela.
**Tentativa descartada no meio do caminho:** um `db/montar.sql` com uma funcao por secao, gerado de `lib/schema.ts`, e um `npm run montar`. Funcionava e foi testado, mas resolvia o problema errado: o usuario nao queria um comando de terminal para escrever viagem, queria pedir aqui e ver aparecer la. Removido antes de qualquer uso.
