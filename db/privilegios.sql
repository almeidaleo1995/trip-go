...
do $$
begin
  if not exists (select 1 from pg_roles where rolname = 'tripgo_app') then
    execute format('create role tripgo_app login password %L', :'senha');
  else
    -- Já existe: só realinha a senha, sem derrubar conexão de ninguém.
    execute format('alter role tripgo_app login password %L', :'senha');
  end if;
end
$$;
