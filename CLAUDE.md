...
- **Sign-in and sign-up are rate-limited by opposite rules.** Sign-in counts only *wrong* attempts and a success clears the window — you are blocking password guessing. Sign-up counts **every** attempt including the successful ones and never clears — the abuse there IS the account that got created. Keys are namespaced so one cannot eat the other's quota.
...
- **`Hoje` converts the clock, never the event.** Times stay local-to-destination as always; `trips.fuso` (IANA, nullable) only turns *the device's now* into the destination's wall clock, for whoever opens the app from home. Null means "trust the device", which is right while travelling.
