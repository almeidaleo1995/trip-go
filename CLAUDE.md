...
- **A message reaches the user only if a person wrote it for a user.** `rota()` turns any non-`ErroHttp` into a generic 500 for exactly this reason, and the two BATCH routes went under it: `/api/mutate` and `/api/assistente/aplicar` report each refused operation in `rejeitadas[].motivo` and were sending `e.message` raw, so a Postgres message handed over the constraint name and the column type (`invalid input syntax for type integer`). `motivoSeguro(e)` is the filter; the real error goes to the log, sanitized, so the diagnosis is not lost.
