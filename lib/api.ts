...
import { ErroHttp, estaBloqueado, registrarFalha, type Limites } from './session.ts'
 ...
 * quem aplica isso é o navegador, e o servidor não tem como saber se aplicou. Esta
 * é a mesma regra conferida do lado que a gente controla, exatamente como
 ...
 */
export function limitar(chave: string, limites: Limites, mensagem: string): void {
  if (estaBloqueado(chave)) throw new ErroHttp(429, mensagem)
  if (registrarFalha(chave, Date.now(), limites).bloqueado) throw new ErroHttp(429, mensagem)
}
